#!/usr/bin/env bash
ROOT_PATH=`pwd`
NGINX_CONFIG_FILE=BlogProject_nginx.conf
NGINX_SITES_CONFIG=/usr/local/etc/nginx/servers
UWSGI_CONFIG_FILE=BlogProject_uwsgi.ini
echo """
# the upstream component nginx needs to connect to
upstream mysite {
    # server 127.0.0.1:8001;
    server unix:///tmp/BlogProject.sock; # for a file socket, more effective
}
